...
class Linear(Neuron):
    ...
    def forward(self):
        ...
        x_val = self.inbound_neurons[0].value * self.weights[0].value
        y_val = self.inbound_neurons[1].value * self.weights[1].value
        z_val = self.inbound_neurons[2].value * self.weights[2].value
        self.value = x_val + y_val + z_val + self.bias.value
        """
